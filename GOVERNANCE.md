...
2. **Bump the version** (semver):
   - **patch** (`1.0.0 → 1.0.1`) — wording, fixes, clarification.
   - **minor** (`1.0.0 → 1.1.0`) — a new skill, or a new capability in an existing one.
   - **major** (`1.0.0 → 2.0.0`) — rename, removal, or a breaking change to triggers/args.
   - Bump `plugin.json` `version` whenever **anything** ships (this is what `/plugin update` keys on).
